...
int main(int argc, char** argv) {
    if (argc < 4) {
        printf("Usage: %s <data_dir> <K> <n_threads>\n", argv[0]);
        return 1;
    }
    ...
    if (n_A != n_samples * n_items || n_S != n_items || n_F != n_items) {
        fprintf(stderr, "Dimension mismatch in loaded files.\n");
        return 1;
    }
    ...
    printf("=== Metagenomic Scoring - OpenMP ===\n");
    printf("Dataset: n_samples=%zu, n_items=%zu\n", n_samples, n_items);
    printf("Candidates K=%zu, Threads=%d\n", K, n_threads);
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp critical
        {
            if (local_best_auc > global_best_auc || (local_best_auc == global_best_auc && local_best_k < global_best_k)) {
                global_best_auc = local_best_auc;
                global_best_w[0] = local_best_w[0];
                global_best_w[1] = local_best_w[1];
                global_best_w[2] = local_best_w[2];
                global_best_k = local_best_k;
            }
        }
    ...
    printf("K=%zu, P=%d, Time=%.4f, AUC=%.6f, W1=%f, W2=%f, W3=%f\n", 
           K, n_threads, search_time, global_best_auc, 
           global_best_w[0], global_best_w[1], global_best_w[2]);
